...
async def worker():
    while True:
        ...
        try:
            ...
            db_entry = session.query(MathOperation).filter_by(id=task["job_id"]).first()
            if db_entry:
                db_entry.result = str(result)  # Store result as string
                db_entry.status = "done"
                session.commit()
                logger.info(f"Task completed with job_id: {task['job_id']}, result: {result}")
            ...
            logger.error(f"Unexpected error for job_id {task['job_id']}: {str(e)}")
            db_entry = session.query(MathOperation).filter_by(id=task["job_id"]).first()
            if db_entry:
